...
from map4_scripts.feature_engineering import select_rgb_white_yellow
...
class ResizeWrapper(gym.ObservationWrapper):
    def __init__(self, env=None):
        ...
        self.shape = (
            int(env.observation_space.shape[0] * custom_params['SCALING_FACTOR']),
            int(env.observation_space.shape[1] * custom_params['SCALING_FACTOR']),
            # env.observation_space.shape[2] * 2,
...
class ModifiedRewardWrapper(gym.RewardWrapper):
    ...
    def reward(self, reward):
        ...
        # except NotInLane:
            ...
            if dist_to_stop < dist_thresh:
                reward = (
                    ...
                    - 10.0 * np.abs(lp.dist)
                    + 40.0 * col_penalty
                    + 1.0 * ((2/(1+self.speed)) - 1) * lp.dot_dir
        ...
        if self.speed > 0.15 and dist_to_stop < dist_thresh:
            # reward = -100.0
            reward -= 1.5
