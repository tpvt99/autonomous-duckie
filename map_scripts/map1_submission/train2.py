...
batch_size = 32
max_episodes = 200000
min_buffer = 99000  # 99000 NEED TO CHANGE THIS
# min_buffer = 150000
# min_buffer = 2  # 99000
...
def compute_epsilon(episode, bias=15000):
...
class ResizeWrapper(gym.ObservationWrapper):
    ...
    #     road_lines = select_rgb_white_yellow(observation)
    #     multi_channel = np.concatenate((observation, road_lines), axis=-1)
    #     crop = multi_channel[100:, :, :]
    #     resized1 = cv2.resize(
    #         crop[:, :, 0:3],
    ...
    #         interpolation=cv2.INTER_AREA,
    #     )
    #     final = np.concatenate((resized1, resized2), axis=-1)
    #     return final
...
def train(model_class, MAP_NAME, MODEL_PATH=MODEL_PATH, seeds_dict=seeds_dict):
    """
    TO DO:
    1) discretize action space for DQN. OK.
    1) ensure reward is working as expected. OK.
    1) adapt the new replay buffer. OK.
    1) Add historical sequence
    2) make sure dqn runs without bugs. OK.
    3) Add cv2 features of road lines etc.

    COMPLEX SETUP:
    1) Follow model in paper
    2) Reward shaping without obstacles

    Details of reward wrapper in: gym-duckietown/gym_duckietown/simulator.py
    """
    ...
    if restore:
        ...
        model = load_model(os.path.join(MODEL_PATH, "run_{}_first_try.pt".format(15000)))
    else:
